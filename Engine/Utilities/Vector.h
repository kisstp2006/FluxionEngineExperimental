...
// Explicit standard includes: this container uses realloc/free (<cstdlib>),
// memcpy (<cstring>), placement new (<new>), forward/move (<utility>) and
// type traits (<type_traits>). MSVC pulls these in transitively via <memory>,
// but libc++/libstdc++ (Linux/macOS/Android/iOS/Web) do not — so include them
...
namespace fluxion::utl {
	...
	public:
		...
		constexpr explicit vector(flu64 count)
		{
			resize(count);
		}

		// Constructor resizes the vector and initializes 'count' items using 'value'.
		constexpr explicit vector(flu64 count, const T& value)
		{
			resize(count, value);
		}
		...
		constexpr void resize(flu64 new_size)
		{
		...
		constexpr void resize(flu64 new_size, const T& value)
		{
		...
		constexpr void reserve(flu64 new_capacity)
		{
		...
		constexpr T *const erase(flu64 index)
		{
		...
		constexpr T *const erase_unordered(flu64 index)
		{
			assert(_data && index < _size);
		...
		[[nodiscard]] constexpr flu64 size() const
		{
			return _size;
		}

		// Returns the current capacity of the vector.
		[[nodiscard]] constexpr flu64 capacity() const
		{
			return _capacity;
		}

		// Indexing operator. Returns a reference to the item at specified index.
		[[nodiscard]] constexpr T& operator[](flu64 index)
		{
			assert(_data && index < _size);
			return _data[index];
		}
		...
		[[nodiscard]] constexpr const T& operator[](flu64 index) const
		{
			assert(_data && index < _size);
			return _data[index];
		}
	...
	private:
		...
		constexpr void destruct_range(flu64 first, flu64 last)
		{
			assert(destruct);
