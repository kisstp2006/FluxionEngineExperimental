...
namespace fluxion::common::id {
	...
	using generation_type = std::conditional_t<detail::generation_bits <= 16, std::conditional_t<detail::generation_bits <= 8, flu8, flu16>, flu32>;
	...
	constexpr id_type new_generation(id_type id) {
		...
		assert(generation < ((flu64)1<<detail::generation_bits)-1);
...
#define DEFINE_TYPED_ID(name)                                          \
	struct name final : fluxion::common::id::detail::id_base         \
	{                                                                   \
